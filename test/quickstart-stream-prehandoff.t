...
# PRE-HANDOFF test:
# Extra-long firehoseGracePeriod so handoff never happens.
my $server_json = JSON::decode_json(
  do { local $/; open my $fh, "<", "$dir/conf-quickstart/tranquility/server.json" or die; <$fh> }
);
$server_json->{'properties'}{'druidBeam.firehoseGracePeriod'} = 'PT10H';
...
# Direct Druid query
my $druid_result = $iap->query_druid(JSON::decode_json(<<EOT), tries => 180);
{
  "queryType" : "topN",
  "dataSource" : "metrics",
  "intervals" : ["$ts/PT1S"],
...
eq_or_diff($druid_result, $druid_expected, 'example druid query results are as expected (before service stop)');

# Restart services
ok($iap->down, 'shut down services');
ok($iap->start('quickstart'), 'restarted services');

# Direct Druid query
my $druid_result2 = $iap->query_druid(JSON::decode_json(<<EOT), tries => 180);
{
  "queryType" : "topN",
  "dataSource" : "metrics",
  "intervals" : ["$ts/PT1S"],
...
eq_or_diff($druid_result2, $druid_expected, 'example druid query results are as expected (after service stop)');
